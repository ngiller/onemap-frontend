      ...
      <k-tab name="DETAIL PEMBIBIT">
        ...
        >
          ...
          <div class="detail-text">
            <div
              v-for="(subitem, x) in getDetailBibitPembibit(item.nama_peserta)"
              :key="x"
              class="detail-bibit"
            >
              {{ subitem.nama }} :
            ...
            <div class="sub-total-bibit">
              Total Bibit :
              <span style="float: right"
                >{{ sumBibit(item.nama_peserta) | numberFormat }} bibit</span
              >
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    updateData: async function(desa_id) {
        ...
        this.data_pembibitan = await this.getDataAPI('desabambu/getpembibitan', desa_id)
        this.jml_bibit = await this.getJumlahBibit()
        this.luas_lahan = await this.getLuasPenanaman('desabambu/getluaslahan', desa_id)
        this.detail_pembibit = await this.getDataAPI('desabambu/getdetailpembibit', desa_id)
        ...
        this.data_kepompong = await this.getDataAPI('desabambu/getkepompong', desa_id)
        this.jml_kepompong = await this.getJumlahKepompong()
        this.luas_kepompong = await this.getLuasPenanaman('desabambu/getluaskepompong', desa_id)

        this.data_penanaman = await this.getDataAPI('desabambu/getasalpenanaman', desa_id)
        this.jml_penanaman = await this.getJumlahPenanaman()
        this.luas_penanaman = await this.getLuasPenanaman('desabambu/getluasasalpenanaman', desa_id)
